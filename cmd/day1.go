...
var day1Cmd = &cobra.Command{
	Use:   "day1",
	Short: "Advent of Code 2024 - Day 1: Historian Hysteria",
	Long: `
--- Day 1: Historian Hysteria ---
You haven't even left yet and the group of Elvish Senior Historians has already hit a problem: their 
list of locations to check is currently empty. Eventually, someone decides that the best place to 
...
This seems to be the planning the Chief Historian was doing before he left. Perhaps these notes can be 
...
apart they are. Pair up the smallest number in the left list with the smallest number in the right list, 
then the second-smallest left number with the second-smallest right number, and so on.

Within each pair, figure out how far apart the two numbers are; you'll need to add up all of those 
distances. For example, if you pair up a 3 from the left list with a 7 from the right list, the distance 
...
Once again consider your left and right lists. What is their similarity score?
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		locIDs, err := helpers.ReadStringFile("inputs/day1.txt")
		if err != nil {
			return err
		}
		...
		rightSideCount := make(map[int]int)
		for _, num := range rightSide {
			rightSideCount[num]++
		}

		for i, leftNum := range leftSide {
			difference := leftNum - rightSide[i]
			totalDistance += int(math.Abs(float64(difference)))
			similarityScore += leftNum * rightSideCount[leftNum]
		}
